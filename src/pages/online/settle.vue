    ...
    <main class='scroll-content-2'>
          ...
          <li v-for="(item,index) in settleList.goodsVos" :key="index" class="order-item flex">
            ...
            <div class="order-info flex-1">
              ...
              <div class="flex just-between">
                <span class="danger-color fs-16" v-if="settleList.isCanUseCou==1">{{item.goodsAmount}}福宝</span>
      ...
      <yd-cell-group>
        ...
        <yd-cell-item>
          ...
          <span slot="right" class="fs-14" v-if="settleList.isCanUseCou==1">{{settleList.totalAmount}}福宝+￥{{formatPrice(settleList.pointNiceAmount)}}</span>
        ...
        <yd-cell-item v-if="orderType=='1'">
          <span slot="left">当前福宝:{{member.consumptionMoney}}</span>
          <span slot="right">
            ...
            <span class="iconfont self-x fs-12 danger-color" v-else>福宝不足</span>
          </span>
        </yd-cell-item>
...
export default {
  ...
  data() {
    return {
      oldBack: mui.back,
      orderType: "0", //0:普通商品 1：福宝兑换 2责任消费 3代金券
      remark: "", //备注
      payType: "", //支付方式
