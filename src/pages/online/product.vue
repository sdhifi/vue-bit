        ...
        <yd-cell-group>
          <template v-if="info.isWholesale=='0'">
            <yd-cell-item v-if="info.isCanUserCou=='1'">
              <span slot="left">福宝使用说明:{{info.productAttrStock&&info.productAttrStock.price}}福宝 +
      ...
      <div class="actionsheet-content" :class="{'actionsheet-toggle':show}">
        <div class="top flex">
          ...
          <div class="info flex flex-1">
            <p>{{info.proName}}</p>
            <template v-if="info.isWholesale=='0'">
              <p class="danger-color fs-14" v-if="info.isCanUserCou=='1'">{{info.productAttrStock&&info.productAttrStock.price}}福宝+￥{{info.pointNeedMoney}}</p>
      ...
      mui.ajax({
        ...
        success(res) {
          ...
          _result.content = _result.content.replace(
            /"\/userfiles/g,
            "\"http://admin.tiantiandui.shop/userfiles"
          );
